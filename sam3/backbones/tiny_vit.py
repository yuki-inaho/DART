...
class TinyViT(nn.Module):
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            ...
            # Update resolution for next layer (simulating PatchMerging Conv2d stride 2 padding 1)
            # Output size = floor((Input + 2*padding - kernel) / stride) + 1
            #             = floor((Input + 2 - 3) / 2) + 1 = floor((Input - 1) / 2) + 1
            if i_layer < self.num_layers - 1:
                curr_resolution = (
                    (curr_resolution[0] - 1) // 2 + 1,
                    (curr_resolution[1] - 1) // 2 + 1,
                )

        # Classifier head
        self.norm_head = (
            nn.LayerNorm(embed_dims[-1]) if num_classes > 0 else torch.nn.Identity()
        )
        self.head = (
