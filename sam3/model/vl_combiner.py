...
class SAM3VLBackbone(nn.Module):
    ...
    def forward_image_part1(self, samples: torch.Tensor, split_block: int = 24) -> dict:
        """Run ViT blocks 0..split_block-1.  Returns intermediate state dict."""
        trunk = self.vision_backbone.trunk
        return trunk.forward_part1(samples, split_block=split_block)

    def forward_image_part2(self, intermediate: dict, split_block: int = 24) -> dict:
        """Run ViT blocks split_block..end + FPN.  Returns standard backbone_out dict."""
        trunk = self.vision_backbone.trunk
        trunk_outputs = trunk.forward_part2(intermediate, split_block=split_block)

        neck = self.vision_backbone
        sam3_features, sam3_pos, _, _ = neck.forward_from_trunk_output(trunk_outputs)
