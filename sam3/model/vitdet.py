...
class Attention(nn.Module):
    ...
    def _apply_rope(self, q, k) -> tuple[Tensor, Tensor]:
        ...
        # Check if precomputed freqs match the actual sequence length.
        # Mismatch happens when input resolution differs from the resolution
        # used to construct the model (e.g. 672px vs 1008px on global
        # attention blocks).  Window attention blocks are unaffected since
        # their token count is always window_size².
        seq_len = q.shape[-2]  # (B, nheads, L, head_dim)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        # Sub-block pruning: skip attention or MLP via residual identity.
        # Set by block_pruner_search.py results or --mask-attn/--mask-mlp CLI.
        self.mask_attn = False
        self.mask_mlp = False

    def forward(self, x: Tensor) -> Tensor:
        # Attention branch (skip if masked → identity via residual)
        ...
        # MLP branch (skip if masked → identity via residual)
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        # Block pruning: set of block indices to skip during forward.
        # Global attention blocks (full_attn_ids) should never be skipped
        # since the last one triggers ln_post + feature extraction.
        self.skip_blocks: set = set()
    ...
    def forward_part1(self, x: torch.Tensor, split_block: int = 24) -> dict:
        """Run patch_embed + pos_embed + ln_pre + blocks[0:split_block].

        Returns dict with intermediate state that can be passed to forward_part2().
        Used by SplitBackboneVideoProcessor to split backbone across pipeline stages.
        """
        x = self.patch_embed(x)
        h, w = x.shape[1], x.shape[2]
    ...
    def forward_part2(
        ...
        outputs = []
        for i in range(split_block, len(self.blocks)):
            if i in self.skip_blocks:
                continue
            x = self.blocks[i](x)
            if (i == self.full_attn_ids[-1]) or (
